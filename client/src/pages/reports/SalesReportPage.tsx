...
export default function SalesReportPage() {
  ...
  const handleExport = () => {
    ...
    setExporting(true);
    try {
      exportSalesReport(data, from, to);
    } finally {
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex gap-2 flex-wrap">
          <button onClick={() => navigate('/reports/turnover')}
            className="text-sm text-gray-500 border border-gray-300 px-3 py-1.5
                       rounded-lg hover:bg-gray-50">
            Stock Turnover →
          </button>
          <button onClick={() => navigate('/reports/delivery')}
            className="text-sm text-gray-500 border border-gray-300 px-3 py-1.5
                       rounded-lg hover:bg-gray-50">
            Delivery Eff. →
          </button>
        ...
        <div className="bg-green-50 border border-green-200 rounded-xl px-4 py-3
          ...
          <div>
            ...
            <p className="text-green-600 text-xs mt-0.5">
              Daily Revenue · By RDC · Top Products · Top Customers
            </p>
          </div>
        </div>
      )}
      ...
      {loading && (
        <div className="p-16 text-center text-gray-400">Loading report…</div>
      )}
      {!loading && error && (
      ...
      {!loading && !error && (
        <>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              { label:'Total Orders',    value: totals.total_orders || 0,
                color:'text-blue-600'   },
              { label:'Total Revenue',   value: formatCurrency(totals.total_revenue || 0),
                color:'text-green-600'  },
              { label:'Avg Order Value', value: formatCurrency(totals.avg_order_value || 0),
                color:'text-purple-600' },
              { label:'Largest Order',   value: formatCurrency(totals.max_order_value || 0),
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {topProducts.slice(0,8).map((p: any, i: number) => (
                      <tr key={p.sku} className="hover:bg-gray-50">
                        ...
                        <td className="px-4 py-2.5 text-gray-500 text-xs font-mono">{p.sku}</td>
